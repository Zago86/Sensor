            ...
            while (active)
            {
                ...
                {                    
                    case STATE_MACHINE_STATE_POLL_LOOP.S_IDLE:
                        {
                            MainWindow.logger.Debug("STATE_MACHINE_STATE.S_IDLE");
                            internalMessageToSend = MainWindow.connectedDevices.MessaggiDaInviare;
                    ...
                    case STATE_MACHINE_STATE_POLL_LOOP.S_READ_MESSAGE:
                        {
                            MainWindow.logger.Debug("STATE_MACHINE_STATE.S_READ_MESSAGE");                            
                            int iRet = -1;
                            switch (tempMessage.DeviceInfo.sensor_connectionType)
                            {
                                case "RS485":
            ...
            while (active)
            {
                switch (stateConvertDataAndSendToDB)
                {
                    ...
                    case STATE_MACHINE_STATE_CONVERT_DATA_SEND_DB.CONVERT_MESSAGE:
                        {
                            ...
                            if (tempMessageToConvert.messageToRead != null)
                            {
                                ...
                                MainWindow.logger.Info("message Slave: {0} convertito", tempMessageToConvert.DeviceInfo.sensor_slaveAddress);
                    ...
                    case STATE_MACHINE_STATE_CONVERT_DATA_SEND_DB.SEND_TO_DB:
                        {
                            MainWindow.DataBase.SendToDB(dataForDB);
